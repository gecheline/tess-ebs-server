...
# histogram
# fig_h = px.histogram(df, x=["width1_2g", "width1_pf"])

# scatter plot
# fig_s = px.scatter(df, x='width1_2g', y='width1_pf', color=np.log10(df['period']))
# fig_s.update_layout(coloraxis_colorbar=dict(
#     title="log_period"))

# coordinates
ra = coord.Angle(catalog['ra'].fillna(np.nan)*u.degree)
ra = ra.wrap_at(180*u.degree)
dec = coord.Angle(catalog['dec'].fillna(np.nan)*u.degree)

# fig_c = px.scatter_geo(catalog, lon='ra', lat='dec', color=np.log10(catalog['period']), projection='mollweide',
#                        basemap_visible=False, labels=['ra', 'dec']
#                        )
# fig_c = make_coords_plot(catalog, x='ra', y='dec')

# light curve
# tic = df['TIC'][0]
# lc_df = load_lc(tic)
# fig_lc = px.scatter(lc_df, x='phases', y='fluxes')
fig_lc = go.Figure()
...
def update_lc(fig_lc, click_data, phasevstime, models, eclipses):
    if click_data is None:
        ...
        if '2g' in eclipses and phasevstime == 'phase':
            ...
            fig_lc.add_vline(x=pos1)
            fig_lc.add_vline(x=pos2)
            for edge in edges:
                fig_lc.add_vline(x=edge)
        ...
        if 'pf' in eclipses and phasevstime == 'phase':
            row = df[df['TIC'] == tic]
            fig_lc.add_vline(x=row['pos1_pf'].values[0])
            fig_lc.add_vline(x=row['pos2_pf'].values[0])
...
def update_coordinates(xcolumns, xtype, coords_type):
    ...
    if xtype == 'log':
        colors = np.log10(catalog[xcolumns[0]])
        label = 'log_' + xcolumns[0]
    else:
        colors = catalog[xcolumns[0]]
        label=xcolumns[0]
        
    lon = coord.Angle(data[x].fillna(np.nan)*u.degree)
    lon = lon.wrap_at(180*u.degree)
    lat = coord.Angle(data[y].fillna(np.nan)*u.degree)
